        ...
        <div className="sticky top-32 space-y-5">
            ...
            >
                ...
                <div className="absolute inset-0 flex items-center justify-center pb-8">
                    <div className="bg-white px-5 py-2.5 rounded-full shadow-md flex items-center gap-2 text-[rgb(40,169,224)] font-bold text-sm transform group-hover:-translate-y-1 transition-transform duration-300 ring-4 ring-[rgb(40,169,224)]/10">
                        <MapIcon size={18} /> {t('hotels.map_view')}
                    </div>
                </div>
            ...
            <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                    <h4 className="font-bold text-slate-800">{i18n.language === 'vi' ? 'Điểm nổi bật' : 'Highlights'}</h4>
                    {rating && (
                ...
                <ul className="space-y-3">
                    <li className="flex items-start gap-3 text-sm text-slate-600">
                        ...
                        <span>{i18n.language === 'vi' ? 'Vị trí trung tâm, thuận tiện di chuyển' : 'Central location, easy to travel'}</span>
                    </li>
                    <li className="flex items-start gap-3 text-sm text-slate-600">
                        <div className="mt-0.5 p-1 rounded-full bg-green-100 text-green-600">
                            <Check size={10} strokeWidth={4} />
                        </div>
                        <span>{i18n.language === 'vi' ? 'Lễ tân phục vụ 24/7' : '24/7 Front desk'}</span>
                    </li>
                    <li className="flex items-start gap-3 text-sm text-slate-600">
                        <div className="mt-0.5 p-1 rounded-full bg-green-100 text-green-600">
                            <Check size={10} strokeWidth={4} />
                        </div>
                        <span>{i18n.language === 'vi' ? 'Hủy phòng miễn phí (tuỳ chính sách)' : 'Free cancellation (subject to policy)'}</span>
                    </li>
                </ul>
            </div>

            <div className="bg-white p-4 rounded-2xl border border-gray-200 shadow-sm text-center space-y-3">
                <p className="text-sm text-slate-500">{i18n.language === 'vi' ? 'Giá tốt nhất chỉ từ' : 'Best price from'}</p>
                <p className="text-2xl font-extrabold text-[rgb(40,169,224)]">
                    {formatPrice(minPrice, convertedMinPrice, currency)}
                    <span className="text-sm font-normal text-slate-400">{t('hotel_detail.per_night')}</span>
                </p>
                <button 
                ...
                >
                    {i18n.language === 'vi' ? 'Chọn phòng ngay' : 'Select Room Now'}
                </button>
            </div>
        </div>
    </div>
